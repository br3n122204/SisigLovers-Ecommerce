...
export default function AnnouncementBar() {
  return (
    <div className="w-full bg-black text-white overflow-hidden whitespace-nowrap border-b border-gray-800">
      <div className="animate-marquee flex items-center" style={{ minWidth: '100%' }}>
        {phrases.map((phrase, idx) => (
          <span key={idx} className="mx-8 text-sm font-medium tracking-wide text-white">
            {phrase}
          </span>
        ))}
        {/* Repeat for seamless loop */}
        {phrases.map((phrase, idx) => (
          <span key={phrases.length + idx} className="mx-8 text-sm font-medium tracking-wide text-white">
            {phrase}
          </span>
        ))}
      </div>
